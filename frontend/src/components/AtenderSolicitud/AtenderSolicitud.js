...
import { aceptarSolicitud, getSolicitudPorId, rechazarSolicitud, vertificarDisponibilidad } from "../../services/AtenderSolicitud.service";

const AtenderSolicitud = ({ solicitudId }) => {
    const [show, setShow] = useState(false);
    const [razonRechazo, setRazonRechazo] = useState();
    const [mostrarMensajeDeVerificacion, setMostrarMensajeDeVerificacion] = useState(null);
    ...
    const onClickRechazarSolicitud = async () => {
        ...
        if (response !== null) {
            agregarAlert({ icon: <CheckCircleFill />, severidad: "success", mensaje: "Solicitud rechazada enviada." });
        } else {
            agregarAlert({ icon: <ExclamationCircleFill />, severidad: "danger", mensaje: "No se pudo enviar la solicitud rechazada." });
        }
    }
    ...
    const onClickAceptarSolicitud = async () => {
        ...
        if (response !== null) {
            agregarAlert({ icon: <CheckCircleFill />, severidad: "success", mensaje: response.mensaje });
        } else {
            agregarAlert({ icon: <ExclamationCircleFill />, severidad: "danger", mensaje: "No se pudo enviar la solicitud aceptada." });
        }
    }
            ...
            <Row>
                <Col sm="4">
                    <Row sm className="text-white AtenderSolicitud-header">
                        <Col xs="12" className="d-flex justify-content-start align-items-center" style={{ height: '100%' }}>
                            <h3 style={{ fontWeight: "bold" }} className="">Atender Solicitud</h3>
                        </Col>
                    </Row>
                            ...
                            <Container fluid>
                                ...
                                <Row xs="auto" sm="auto" style={{ paddingTop: "1rem" }} className="justify-content-end">
                                    ...
                                    <Button
                                        ...
                                        onClick={onClickParaVerificarDisponibilidad}>Verificar Disponibilidad</Button>
                                </Row>
                                <Row xs="auto" style={{ paddingTop: "1rem" }}>
                                    <Stack direction="horizontal" gap={2} >
                                        <Button
                                            ...
                                            onClick={() => setShow(!show)}>Rechazar</Button>
                                        <Button
                                            size="sm"
                                            className="btn AtenderSolicitud-button-aceptar"
                                            onClick={onClickAceptarSolicitud}>Aceptar</Button>
                                    </Stack>
                                </Row>
                            </Container>
                        </Col>
        ...
        >
            <Row sm className="text-white AtenderSolicitud-modal-header">
                <Col xs="10" className="d-flex justify-content-start align-items-center" style={{ height: '100%' }}>
                    <h4 style={{ fontWeight: "bold" }} className="">Razon de rechazo</h4>
                </Col>
